"""Contains the bio_anotator class, which exposes most of Sabers functionality.
"""
import logging
import os
import pickle
...
print('bio_anotator version: {0}'.format(constants.__version__))
LOGGER = logging.getLogger(__name__)


class bio_anotator(object):
    """The interface for bio_anotator.

    As the interface to bio_anotator, this class exposes all of Sabers functionality, including the
    ...
    def annotate(self, text, title='', model_idx=0, jupyter=False, coref=False, ground=False):
        ...
        if ground:
            try:
                ...
                err_msg = ("Grounding step in `bio_anotator.annotate()` failed (`grounding_utils.ground()`"
    ...
    def save(self, directory=None, compress=True, model_idx=0):
        """Coordinates the saving of bio_anotator models.
    ...
    def load(self, directory):
        """Coordinates the loading of bio_anotator models.

        Loads a bio_anotator model saved at `directory`. Creates and compiles a new model with identical
    ...
    def load_embeddings(self, filepath=None, binary=True, load_all=None):
        ...
        if not self.config.pretrained_embeddings:
            err_msg = ("`bio_anotator.load_embeddings()` was called but `pretrained_embeddings` argument "
                       "is empty")
            LOGGER.error('ValueError: %s', err_msg)
            raise ValueError(err_msg)
    ...
    def build(self, model_name=None):
        ...
        if not self.datasets:
            err_msg = "You need to call 'bio_anotator.load_dataset()' before calling 'Saber.build()'"
            LOGGER.error('MissingStepException: %s', err_msg)
            raise MissingStepException(err_msg)
    ...
    def train(self):
        ...
        """
        if not self.datasets:
            err_msg = "You need to call `bio_anotator.load_dataset()` before calling `Saber.train()`"
            LOGGER.error('MissingStepException: %s', err_msg)
            raise MissingStepException(err_msg)
        if not self.model:
            err_msg = "You need to call `bio_anotator.build()` before calling `Saber.train()`"
            LOGGER.error('MissingStepException: %s', err_msg)
            raise MissingStepException(err_msg)
